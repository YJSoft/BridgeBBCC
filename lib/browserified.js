...
var replaceMsgFormat = function(message, amount) {
	...
	var retMessage = message.replace(/{(months|bits|raid)}/g, amount);
    if (amount == 0) {
...
addChatMessage = function(nick, message, data) {
    ...
    if (data.nick) {
        chatOuterBox.classList.add("user_" + data.nick);
    }
    ...
    if (typeof applyMessage != "undefined") {
        ...
        chatMessageBox.innerHTML = message;
    }
    ...
    if (data) {
		...
		if (data.raidCount != undefined) {
			chatMessageBox.innerHTML = "";
            chatLowerBox.innerHTML =
                '<div class="chat_raid_box">' +
                replaceMsgFormat(configData.raidMsg, data.raidCount) +
                "</div>" + chatLowerBox.innerHTML;
        }
    }
    ...
    // 메세지 타임아웃 설정
    var fadeoutTime = 0;
    if (configData.msgExistDuration) {
        fadeoutTime += configData.msgExistDuration * 1000;
    }
    if (configData.msgAniDuration) {
        fadeoutTime += configData.msgAniDuration * 1000;
    }
...
var concatChatMessage = function(nick, message, data) {
    ...
    if (lChild && lChild.getElementsByClassName("chat_nickname_box")[0].innerHTML == nick) {
        if (typeof applyMessage != "undefined") {
            message = applyMessage(message, data);
        }
        ...
        addChatMessage.apply(this, arguments);
    }
}
...
var applyReplace = function(message, data) {
    return message;
};
var applyCheerIcon = function(message, data) {
    return message;
};
var applyTwitchCon = function(message, data) {
    return message;
};
var applyDcCon = function(message, data) {
    return message;
};
var applyMessage = function(message, data) {
    ...
    if (twipApply) {
        message = twipApply.apply(message, data);
    }
    return message;
}
...
debugLog = function(dat) {}; {
    ...
    if (Object.keys(configData).length < Object.keys(configDefault).length) {
        if (Object.keys(configData).length == 0) {
            ...
            configLoadMessage = "일부 설정을 찾을 수 없어 기본값을 사용합니다. ";
        }
...
var loadCss = function(isRetrying) {
    ...
    document.head.appendChild(function() {
        if (configData.themeURI == "") {
            configData.themeURI = "./theme/";
...
if (configData.loadCheerImgs) {
    applyCheerIcon = function(message, data) {
        ...
        if (matches.match(testRegExp) == null) {
            return message;
        }
...
if (configData.loadTwitchCons) {
    ...
    if (configData.consRealSubsOnly) {
		...
		debugLog("이 옵션을 제대로 사용하려면 twitchemotes 데이터를 캐시하는 서버를 직접 구동해야 합니다.");
        var isTwitchConsCalled = {
            emotes: false,
            subs: false
        };
        var isTwitchConsLoaded = {
            emotes: false,
            subs: false
        };
        var consIdMap = {};
        ...
        var twitchConsApply = function() {
            ...
            if (isTwitchConsLoaded.subs) {
                if (isTwitchConsLoaded.emotes) {
                    loadMsg = "트위치 이모티콘과 구독콘" + loadMsg;
                } else {
                    loadMsg = "트위치 구독콘" + loadMsg;
                }
            } else {
                if (isTwitchConsLoaded.emotes) {
                    loadMsg = "트위치 이모티콘" + loadMsg;
                } else {
            ...
            if (isTwitchConsLoaded.subs && isTwitchConsLoaded.emotes) {
                checkComplete();
			}
        ...
        twitchEmoteConRequest.onreadystatechange = function(evt) {
            if (twitchEmoteConRequest.readyState == 4) {
                ...
                if (isTwitchConsCalled.emotes && isTwitchConsCalled.subs) {
                    twitchConsApply();
                }
            }
        };
    ...
    var onmessageEventHandler = function(evt) {
        ...
        lines.forEach(function(element) {
            ...
            if (line[0] == ":") {
                ...
                switch (args[1]) {
					case "ROOMSTATE": // 방 상태 - 슬로우 모드, 이모티콘 전용 채팅 여부 등
                        break;
					...
					case "USERNOTICE":
                        ...
                        } else if (twitchArgs["msg-param-login"] && typeof twitchArgs["msg-param-viewerCount"] != 'undefined') {
                            // 레이드 받음
							data.raidCount = Number(twitchArgs["msg-param-viewerCount"]);
							data.raidFrom = twitchArgs["msg-param-displayName"] ? twitchArgs["msg-param-displayName"] : twitchArgs["msg-param-login"];
                        } else {
                            if (configData.allMessageHandle) {
                                debugLog("처리되지 않은 메세지를 수신했습니다.<br />" + line);
                            }
